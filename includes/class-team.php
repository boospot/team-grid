...
class Team {
	...
	public function create_custom_post_type() {
		...
		$post_type_slug = ( ! empty( $this->get_options_value( 'team_grid_member_slug' ) ) )
			? sanitize_key( $this->get_options_value( 'team_grid_member_slug' ) )
			: 'team';

		$team_grid_member_type_slug = ( ! empty( $this->get_options_value( 'team_grid_member_type_slug' ) ) )
			? sanitize_key( $this->get_options_value( 'team_grid_member_type_slug' ) )
			: 'member-type';

		$post_types_fields = apply_filters( 'team_grid_post_type_create_args', array(

			array(
				'slug'                => 'team_grid_member',
				'singular'            => __( 'Team Member', 'team-grid' ),
				'plural'              => __( 'Team Members', 'team-grid' ),
				'menu_name'           => __( 'Team Members', 'team-grid' ),
				'description'         => __( 'Team Members', 'team-grid' ),
				'has_archive'         => true,
				'hierarchical'        => false,
				...
				'custom_caps'         => false,
				'taxonomies'          => apply_filters( 'team_grid_taxonomies_create_args', array(
					array(
						'id'                  => 'team_grid_member_type',
						'taxonomy'            => 'team_grid_member_type',
						'plural'              => __( 'Member Types', 'team-grid' ),
						'single'              => __( 'Member Types', 'team-grid' ),
						'post_types'          => array( 'team_grid_member' ),
						'rewrite'             => array( 'slug' => $team_grid_member_type_slug, 'with_front' => false ),
		...
		foreach ( $post_types_fields as $fields ) {
			...
			$this->register_single_post_type( apply_filters( 'team_grid_post_type_args_before_register', $fields ) );

		}

		do_action( 'team_grid_post_type_create' );

//		$team_grid_add_image_sizes_array = apply_filters( 'team_grid_team_grid_add_image_sizes_array', array(
//			'team_grid_image'                     => array(
//				'name'   => 'team_grid_image',
//				'width'  => 1024,
//				'height' => 576,
//				'crop'   => true
//			),
//			'team_grid_landscape_image'           => array(
//				'name'   => 'team_grid_landscape_image',
//				'width'  => 768,
//				'height' => 500,
//				'crop'   => true
//			),
//			'team_grid_landscape_image_thumbnail' => array(
//				'name'   => 'team_grid_landscape_image_thumbnail',
//				'width'  => 150,
//				'height' => 100,
//				'crop'   => true
//			),
//			'team_grid_landscape_image_archive'   => array(
//				'name'   => 'team_grid_landscape_image_archive',
//				'width'  => 350,
//				'height' => 300,
//				'crop'   => true
//			),
...
//
//
//		foreach ( $team_grid_add_image_sizes_array as $image_size_args ) {
//			add_image_size(
...
//		}
	...
	public function get_options_value( $option_id ) {
		return Globals::get_options_value( 'general', $option_id );
	}
	...
	private function register_single_post_type( $fields ) {
		...
		$labels = array(
			...
			'new_item'              => sprintf( __( 'New %s', 'team-grid' ), $fields['singular'] ),
			'add_new_item'          => sprintf( __( 'Add new %s', 'team-grid' ), $fields['singular'] ),
			'edit_item'             => sprintf( __( 'Edit %s', 'team-grid' ), $fields['singular'] ),
			'view_item'             => sprintf( __( 'View %s', 'team-grid' ), $fields['singular'] ),
			'view_items'            => sprintf( __( 'View %s', 'team-grid' ), $fields['plural'] ),
			'search_items'          => sprintf( __( 'Search %s', 'team-grid' ), $fields['plural'] ),
			'not_found'             => sprintf( __( 'No %s found', 'team-grid' ), strtolower( $fields['plural'] ) ),
			'not_found_in_trash'    => sprintf( __( 'No %s found in trash', 'team-grid' ), strtolower( $fields['plural'] ) ),
			'all_items'             => sprintf( __( 'All %s', 'team-grid' ), $fields['plural'] ),
			'archives'              => sprintf( __( '%s Archives', 'team-grid' ), $fields['singular'] ),
			'attributes'            => sprintf( __( '%s Attributes', 'team-grid' ), $fields['singular'] ),
			'insert_into_item'      => sprintf( __( 'Insert into %s', 'team-grid' ), strtolower( $fields['singular'] ) ),
			'uploaded_to_this_item' => sprintf( __( 'Uploaded to this %s', 'team-grid' ), strtolower( $fields['singular'] ) ),

			/* Labels for hierarchical post types only. */
			'parent_item'           => sprintf( __( 'Parent %s', 'team-grid' ), $fields['singular'] ),
			'parent_item_colon'     => sprintf( __( 'Parent %s:', 'team-grid' ), $fields['singular'] ),
...
//				'custom-fields',
	...
	public function register_single_post_type_taxonomy( $tax_fields ) {
		...
		$labels = array(
			...
			'all_items'                  => sprintf( __( 'All %s', 'team-grid' ), $tax_fields['plural'] ),
			'edit_item'                  => sprintf( __( 'Edit %s', 'team-grid' ), $tax_fields['single'] ),
			'view_item'                  => sprintf( __( 'View %s', 'team-grid' ), $tax_fields['single'] ),
			'update_item'                => sprintf( __( 'Update %s', 'team-grid' ), $tax_fields['single'] ),
			'add_new_item'               => sprintf( __( 'Add New %s', 'team-grid' ), $tax_fields['single'] ),
			'new_item_name'              => sprintf( __( 'New %s Name', 'team-grid' ), $tax_fields['single'] ),
			'parent_item'                => sprintf( __( 'Parent %s', 'team-grid' ), $tax_fields['single'] ),
			'parent_item_colon'          => sprintf( __( 'Parent %s:', 'team-grid' ), $tax_fields['single'] ),
			'search_items'               => sprintf( __( 'Search %s', 'team-grid' ), $tax_fields['plural'] ),
			'popular_items'              => sprintf( __( 'Popular %s', 'team-grid' ), $tax_fields['plural'] ),
			'separate_items_with_commas' => sprintf( __( 'Separate %s with commas', 'team-grid' ), $tax_fields['plural'] ),
			'add_or_remove_items'        => sprintf( __( 'Add or remove %s', 'team-grid' ), $tax_fields['plural'] ),
			'choose_from_most_used'      => sprintf( __( 'Choose from the most used %s', 'team-grid' ), $tax_fields['plural'] ),
			'not_found'                  => sprintf( __( 'No %s found', 'team-grid' ), $tax_fields['plural'] ),
		);
	...
	public function register_meta_box_primary( $meta_boxes ) {
		...
		$team_grid_primary_fields = apply_filters( 'team_grid_team_grid_metabox_fields', array(
			array(
				'id'   => $prefix . 'job_title',
				'name' => esc_html__( 'Job Title', 'team-grid' ),
			),
			array(
				'id'   => $prefix . 'intro',
				'name' => esc_html__( 'Introduction', 'team-grid' ),
				'type' => 'textarea',
			),

			array(
				'id'   => $prefix . 'short_bio',
				'name' => esc_html__( 'Short Bio', 'team-grid' ),
				'type' => 'textarea',
			),
			array(
				'id'               => $prefix . 'grid_image',
				'type'             => 'image_advanced',
				'name'             => esc_html__( 'Thumbnail in Grid', 'team-grid' ),
				'max_file_uploads' => 1
			),
		...
		$primary_metabox_array = array(
			'id'         => 'team-grid-member-primary',
			'title'      => esc_html__( 'Team Member Information', 'team-grid' ),
			'post_types' => array( 'team_grid_member' ),
			'context'    => 'normal',
			'priority'   => 'high',
			'autosave'   => 'false',
//			'style'      => 'seamless',
			'fields'     => $team_grid_primary_fields,
		);
